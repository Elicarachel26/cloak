        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('product.create') }}" class="btn btn-primary btn-label waves-effect waves-light">
                    <i class="ri-add-line label-icon align-middle fs-16 me-2"></i>Add New Product
                    </a>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <img class="img-thumbnail" alt="{{ $item->name }}" width="150"
                                     src="{{ !empty($item->picture) ? url('storage/product/' . $item->picture) : asset('assets/images/no-image.jpg') }}">
                                </td>
                                <td>{{ $item->name ?? '-' }}</td>
                                <td>{{ $item->description ?? '-' }}</td>
                                <td class="text-center">
                                    <div class="badge badge-soft-{{ $item->status ? 'success' : 'danger' }} text-uppercase">{{ $item->status ?  'Active' : 'Inactive' }}</div>
                                </td>
                                <td>
                                    <div class="d-flex justify-content-center gap-1">
                                        ...
                                        <button type="button"
                                            ...
                                            onclick="hapus('Product', '{{ $item->name }}', '{{ route('product.destroy', $item->id) }}', '{{ csrf_token() }}')"><i
